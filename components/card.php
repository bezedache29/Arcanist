...
<div class="bg-arcane-800 rounded-xl overflow-hidden border border-arcane-700 hover:border-mystic-500/50 transition-all shadow-lg group">
    <div class="h-52 bg-arcane-900 relative flex items-center justify-center overflow-hidden">
        <?php if ($image): ?>
            <img src="<?= htmlspecialchars($image, ENT_QUOTES, 'UTF-8') ?>" alt="<?= htmlspecialchars($title, ENT_QUOTES, 'UTF-8') ?>" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
        <?php else: ?>
    ...
    <div class="p-5 flex flex-col gap-4">
        <div>
            <h3 class="font-grimoire text-xl text-mystic-900 dark:text-mystic-100 group-hover:text-mystic-500 transition-colors"><?= htmlspecialchars($title, ENT_QUOTES, 'UTF-8') ?></h3>
            <p class="text-xs text-mystic-900/40 dark:text-mystic-100/40 mt-1 uppercase tracking-wider font-semibold">Ref: <?= htmlspecialchars($ref, ENT_QUOTES, 'UTF-8') ?></p>
        </div>
