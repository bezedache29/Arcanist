...
<div class="mb-8">
    ...
    <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100 mt-4">Ajouter un produit</h1>
    <p class="mt-1 text-sm text-slate-700 dark:text-slate-400">Remplissez les informations ci-dessous pour créer un nouveau produit.</p>
</div>
...
<div class="bg-white dark:bg-slate-800 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700 sm:rounded-xl overflow-hidden transition-colors max-w-3xl">
    <div class="p-6 sm:p-8">
        ...
        <form action="/pages/admin/products/products_create.php" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="<?= htmlspecialchars($formData['name'] ?? '') ?>" required class="mt-1 block w-full rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 py-2 px-3 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 text-slate-900 dark:text-white transition-colors">
            </div>

            <div>
                <label for="image-input" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Image du produit</label>
                <div class="mt-2 flex items-center gap-6">
                    <div id="preview-container" class="hidden shrink-0">
                        <img id="image-preview" src="#" alt="Aperçu" class="h-24 w-24 object-cover rounded-lg ring-1 ring-slate-200 dark:ring-slate-700 shadow-sm">
                    </div>
                    <div id="preview-placeholder" class="shrink-0 h-24 w-24 flex items-center justify-center rounded-lg border-2 border-dashed border-slate-300 dark:border-slate-600 text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-slate-800/50 transition-colors">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="mt-2 text-xs text-slate-500 dark:text-slate-400">Formats acceptés : JPG, PNG, WEBP. Taille max : 2 Mo.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" step="0.01" min="0" name="price" id="price" value="<?= htmlspecialchars($formData['price'] ?? '') ?>" required class="mt-1 block w-full rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 py-2 px-3 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 text-slate-900 dark:text-white transition-colors">
                </div>

                <div>
                    <label for="stock" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Stock initial <span class="text-red-500">*</span></label>
                    <input type="number" step="1" min="0" name="stock" id="stock" value="<?= htmlspecialchars($formData['stock'] ?? '') ?>" required class="mt-1 block w-full rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 py-2 px-3 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 text-slate-900 dark:text-white transition-colors">
                </div>
            </div>
            ...
            <div class="border-t border-slate-200 dark:border-slate-700 pt-6">
                ...
                <?php if (empty($categories)): ?>
                    <p class="text-sm text-slate-500 dark:text-slate-400 italic">Aucune catégorie disponible. Veuillez en créer une dans le menu Catégories.</p>
                <?php else: ?>
                            ...
                            <label class="relative flex items-start cursor-pointer">
                                <div class="flex h-6 items-center">
                                    <input type="checkbox" name="category_ids[]" value="<?= $category['id'] ?>"
                                        <?= in_array($category['id'], $formData['category_ids'] ?? []) ? 'checked' : '' ?>
                                        class="h-4 w-4 rounded border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-blue-600 focus:ring-blue-600 transition-colors">
                                </div>
